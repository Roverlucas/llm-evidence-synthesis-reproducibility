...
\begin{table}[htbp]
\centering
\caption{Seed effect vs.\ deployment effect. Cloud models tested: Claude Sonnet~4.5 (no seed parameter), Gemini~2.5~Pro and GPT-4.1 (seed=42). Local models all use seed=42.}
\label{tab:seed}
\begin{tabular}{lccc}
\toprule
...
\noindent Seed effect for extraction (cloud-seeded vs.\ cloud-unseeded): $+0.125$ EMR.
Deployment effect (local-seeded vs.\ cloud-seeded): $+0.825$ EMR. The deployment effect is \textbf{6.6$\times$ larger} than the seed effect. We conclude that the seed parameter, as currently implemented in commercial APIs, does not deliver determinism; deployment paradigm dominates.
