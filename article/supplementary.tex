...
\noindent For screening decisions, all six deployment stacks reach the Cochrane $\kappa{\geq}0.80$ guideline for human inter-rater agreement, with all three local stacks attaining the trivial upper bound $\kappa{=}1.000$ and the three cloud APIs in the ``almost perfect'' band ($\kappa{\in}[0.974, 0.981]$).
...
This study was not formally pre-registered. The original experiments (main text \S4.1--\S4.6) are exploratory: 10 repetitions, whole-output hash EMR, four semantic-tier definitions, and fixed-effect meta-analytic pooling were chosen during analysis design but without an OSF or AsPredicted submission. The post-revision analyses (\S4.7 fixed-slot, \S4.8 desconfound, \S5.5 cost-benefit, dual-human gold validation, silver-external) were designed and committed to the public Git repository \emph{before} result computation.
...
We acknowledge the absence of formal pre-registration as a limitation and recommend that future LLM-in-SR studies pre-register analyses on OSF before data collection.
