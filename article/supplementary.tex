...
We applied Holm-Bonferroni (FWER) and Benjamini-Hochberg (FDR) corrections to the 21 pairwise field-level EMR contrasts between the three cloud models. Of 21 contrasts: 7 are significant at raw $p{<}0.05$, 3 survive Holm correction, 5 survive BH-FDR.

\begin{table}[htbp]
\centering
\caption{Pairwise contrasts of cloud-API field-level EMR surviving Holm-Bonferroni correction at $\alpha{=}0.05$.}
\label{tab:mcc}
\begin{tabular}{llccc}
\toprule
\textbf{Field} & \textbf{Models} & \textbf{$\Delta$ EMR} & \textbf{Raw $p$} & \textbf{Holm $p$} \\
\midrule
study\_location           & Claude vs.\ GPT-4.1   & $-0.220$ & $<\!0.0001$ & $\mathbf{<\!0.0001}$ \\
study\_location           & Gemini vs.\ GPT-4.1   & $-0.104$ & $0.001$ & $\mathbf{0.018}$ \\
extraction\_overall\_EMR  & Claude vs.\ Gemini    & $-0.150$ & $0.001$ & $\mathbf{0.026}$ \\
\bottomrule
\end{tabular}
\end{table}

\noindent The remaining 18 contrasts (population, sample\_size, study\_design, study\_period, screening overall, etc.) are not statistically robust after FWER correction; the original ``distinct instability profiles'' claim (main text Finding~3) is therefore tightened to: \emph{differences are robust for study\_location and overall extraction EMR, but not for other categorical fields}.
...
\begin{table}[htbp]
\centering
\caption{Random-effects (DerSimonian-Laird) pooled RR variation across 200 random subsamples per $k$, seed=42, computed independently for each of 10 LLM runs. ``Unstable null-crossing'' = subsamples where the pooled 95\% CI crosses RR=1 in some runs but not others (i.e., the meta-analytic conclusion changes depending on the LLM run).}
\label{tab:smalllit}
\begin{tabular}{lcccc}
\toprule
\textbf{Model} & \textbf{$k$} & \textbf{Mean range} & \textbf{P95 range} & \textbf{\% unstable null-cross} \\
\midrule
Claude Sonnet 4.5 & 10 & 0.0082 & 0.0292 & \textbf{0.50\%} \\
Claude Sonnet 4.5 & 15 & 0.0081 & 0.0237 & 0.00\% \\
Claude Sonnet 4.5 & 20 & 0.0078 & 0.0197 & 0.00\% \\
Gemini 2.5 Pro    & 10 & 0.0167 & 0.0471 & \textbf{0.50\%} \\
Gemini 2.5 Pro    & 15 & 0.0131 & 0.0350 & 0.00\% \\
Gemini 2.5 Pro    & 20 & 0.0110 & 0.0227 & 0.00\% \\
GPT-4.1           & 10 & 0.0063 & 0.0263 & 0.00\% \\
GPT-4.1           & 15 & 0.0054 & 0.0150 & 0.00\% \\
GPT-4.1           & 20 & 0.0065 & 0.0149 & 0.00\% \\
\bottomrule
\end{tabular}
\end{table}
