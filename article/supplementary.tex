...
Per RSM P1.b audit, the original two-proportion $z$-test was replaced with McNemar's test because the contrasts compare \emph{paired} per-item consistency outcomes on the same 100 articles (or 500 abstracts) across cloud APIs.
...
Per RSM P1.c audit, we report Fleiss' $\kappa$ for inter-run agreement on screening decisions and extraction outputs, treating each of the 10 LLM runs as one rater. Fleiss' $\kappa$ adjusts the raw concordance for the agreement expected by chance under fixed-marginal multinomial sampling, providing a more interpretable benchmark than EMR alone (which conflates ``perfectly consistent'' with ``trivially consistent in a degenerate-marginals regime''). Asymptotic 95\% CIs use the closed-form variance of Fleiss (1971, eq.~13).

\begin{table}[htbp]
\centering
\caption{Fleiss' $\kappa$ (95\% CI) for inter-run agreement across 10 runs per deployment stack. Screening: 3-class decisions (include/exclude/uncertain) over items present in all 10 runs. Extraction (hash): each unique output hash treated as a distinct category. Extraction (per categorical field): study\_design and population. Landis \& Koch (1977) reference bands: $<0.0$ poor, $0.0$--$0.2$ slight, $0.21$--$0.4$ fair, $0.41$--$0.6$ moderate, $0.61$--$0.8$ substantial, $0.81$--$1.0$ almost perfect.}
\label{tab:fleiss}
\begin{tabular}{lcccc}
\toprule
\textbf{Deployment Stack} & \textbf{Screening (3-class)} & \textbf{Extraction (hash)} & \textbf{study\_design} & \textbf{population} \\
\midrule
LLaMA 3 8B / Ollama / M4       & $1.000\,[0.975, 1.025]$ & $1.000\,[0.997, 1.003]$ & $1.000\,[0.942, 1.058]$ & $1.000\,[0.952, 1.048]$ \\
Mistral 7B / Ollama / M4       & $1.000\,[0.896, 1.104]$ & $1.000\,[0.997, 1.003]$ & $1.000\,[0.945, 1.055]$ & $1.000\,[0.956, 1.044]$ \\
Gemma 2 9B / Ollama / M4       & $1.000\,[0.965, 1.035]$ & $1.000\,[0.997, 1.003]$ & $1.000\,[0.945, 1.055]$ & $1.000\,[0.951, 1.049]$ \\
\midrule
Claude Sonnet 4.5 / Anthropic  & $0.981\,[0.965, 0.998]$ & $\mathbf{0.180\,[0.178, 0.181]}$ & $0.985\,[0.933, 1.037]$ & $0.974\,[0.923, 1.024]$ \\
Gemini 2.5 Pro / Google        & $0.975\,[0.955, 0.995]$ & $\mathbf{0.505\,[0.502, 0.507]}$ & $1.000\,[0.949, 1.051]$ & $0.979\,[0.937, 1.020]$ \\
GPT-4.1 / OpenAI               & $0.974\,[0.959, 0.989]$ & $\mathbf{0.512\,[0.509, 0.514]}$ & $0.990\,[0.937, 1.044]$ & $0.997\,[0.949, 1.044]$ \\
\bottomrule
\end{tabular}
\end{table}
...
For \emph{whole-output} extraction hashes, however, Fleiss' $\kappa$ collapses for the cloud APIs to $0.180$ (Claude, ``slight''), $0.505$ (Gemini, ``moderate''), and $0.512$ (GPT-4.1, ``moderate''). The Claude value is particularly stark: when whole-output hash is the rating, Claude's 10 runs agree at only the ``slight'' level on the Landis-Koch scale, well below the Cochrane $\kappa{\geq}0.80$ threshold. Per-field $\kappa$ on categorical extraction fields (study\_design, population) remains in the ``almost perfect'' band for all stacks, isolating the source of low whole-output $\kappa$ to the variable-length \texttt{estimates} array (already documented in Section~\ref{main-sec:fieldlevel} of the main text).
...
Per RSM P2.a audit, we re-ran the small-literature simulation with the Hartung-Knapp-Sidik-Jonkman (HKSJ) variance correction alongside the original DL/Wald-z formulation.
